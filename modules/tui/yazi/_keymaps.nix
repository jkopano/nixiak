  ...
  mgr = {
    keymap = [
      ...
      }
      {
        on = "e";
        run = ''
          shell --block '$EDITOR "$1"'
        '';
        desc = "Edit the file in EDITOR";
      }
      {
        on = "o";
        run = "open";
      ...
      }
      {
        on = "f";
        run = ''
          shell --block "res=\$(fd . -H --max-depth 1 | fzf --reverse --preview=\"[[ -d {} ]] && ls -la {} || bat {}\" --header='Jump to location' | sed 's/\\\\/\\\\\\\\/g;s/\"/\\\\\"/g'); if [ -d \"\$res\" ]; then ya emit cd \"\$res\"; elif [ -f \"\$res\" ]; then ya emit reveal \"\$res\"; fi"
        '';
        desc = "Jump to file/dir with custom fzf";
      }
      # {
      #   on = "Z";
      #   run = "export FZF_DEFAULT_COMMAND='fd -d 3' plugin fzf";
