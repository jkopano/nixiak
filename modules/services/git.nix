{ den, ... }:
{
  den.aspects.services._.git.homeManager =
    { pkgs, lib, ... }:
    let
      username = "kuba";
            ...
            customCommands = [
              {
                ...
                command = ''
                  aichat "Please suggest 10 commit messages, given the following diff:
                          ...
                          \`\`\`diff
                          $(${lib.getExe pkgs.git} diff --cached)
                          \`\`\`
                          ...
                          **Output Template**
                          ...
                          numbers or other decorations. Do not include numbers, bullets, markdown, or explanations.
                          Output only raw commit messages, one per line.

                          DO NOT INCLUDE ANY OTHER TEXT LIKE:
                          - here are the commit messages i generated.
                          - I hope it helps
                          - etc.

                          I WANT ONLY THE COMMIT MESSAGES LIKE IN PATTERN BELLOW:
                          ...
                          **Instructions:**
                          ...
                          Write your 10 commit messages below in the format shown in Output Template section above." \
                            | ${lib.getExe pkgs.fzf} --height 40% --border --ansi --preview "echo {}" --preview-window=up:wrap \
                            | xargs -I {} bash -c '
                                COMMIT_MSG_FILE=$(mktemp)
                                echo "{}" > "$COMMIT_MSG_FILE"
                                ''${EDITOR:-vim} -c "cd /tmp" "$COMMIT_MSG_FILE"
